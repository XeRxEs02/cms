...
const InventoryItemDetails = (props) => {
  const { sandData, setSearchItemTerm, searchItemTerm, searchItemResults } =
    props;
  return (
    <div>
      <div className="overflow-x-auto rounded-md">
        <table className="w-full text-left text-sm text-gray-600 border border-gray-200 rounded-md">
          ...
          <tbody className="bg-white divide-y divide-gray-200">
            {(searchItemTerm ? searchItemResults : sandData).map((item) => (
              <tr
                key={item.id}
              ...
              >
                ...
                <td className="px-4 py-3">{item.drNo}</td>
